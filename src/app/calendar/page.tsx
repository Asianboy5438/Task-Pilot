...
import { ChevronLeft, ChevronRight, X, ChevronDown, Loader2 } from "lucide-react";
...
function priorityColor(p: string) {
  const norm = p === "HIGH" ? "High" : p === "MEDIUM" ? "Medium" : p === "LOW" ? "Low" : p;
  if (norm === "High")   return "bg-red-500/20 text-red-400 border-red-500/30";
  if (norm === "Medium") return "bg-yellow-500/20 text-yellow-400 border-yellow-500/30";
...
export default function CalendarPage() {
  ...
  const handleAdd = async () => {
    ...
    const res = await fetch("/api/tasks", {
      ...
      body: JSON.stringify({
        ...
        className: newTaskClass.trim() || "General",
        notes: newTaskDetails.trim(),
    ...
    if (res.ok) {
      ...
      setTasks(prev => [...prev, { ...newTask, dueTime: newTaskTime || "" }]);
    }
    closeModal();
  };
  ...
  // Match task dueDate to calendar date string
  ...
  const TaskPill = ({ task, compact = false }: { task: Task; compact?: boolean }) => (
    ...
    >
      ...
      {task.title}
      {!compact && (task.className || task.class) && (task.className || task.class) !== "General" && (
        <span className="ml-1 opacity-50 text-[10px]">· {task.className || task.class}</span>
      )}
    </div>
  );
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-slate-500 mt-1 text-sm">Click any day to add a task.</p>
          </div>
          <div className="flex items-center gap-3">
            {/* View toggle */}
            <div className="flex items-center gap-1 bg-[var(--bg-header)] border border-[var(--border-color)] rounded-xl p-1">
              {(["month", "week", "day"] as ViewMode[]).map(v => (
                <button
                  key={v}
                  onClick={() => setView(v)}
                  className={`px-4 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all ${
                    view === v ? "bg-indigo-600 text-white shadow" : "text-slate-400 hover:text-[var(--text-strong)]"
                  }`}
                >
                  {v}
                </button>
            ...
            <span className="text-lg font-bold text-[var(--text-strong)] w-64 text-center">{navLabel()}</span>
              ...
              return (
                <div key={dateStr} className="flex flex-col gap-2">
                  <div className={`text-center py-2 rounded-xl text-xs font-black uppercase tracking-widest ${
                    isToday ? "bg-indigo-600 text-white" : "text-slate-400 bg-[var(--bg-header)] border border-[var(--border-color)]"
                  }`}>
            ...
            <div className="rounded-2xl border border-[var(--border-color)] bg-[var(--bg-header)] overflow-hidden">
              ...
              <div className="p-6 space-y-3">
                {dayTasks.length === 0 ? (
                  ...
                  dayTasks.map(task => (
                    <div key={task.id} className={`flex items-start gap-4 p-4 rounded-xl border ${priorityColor(task.priority)} ${(task.isComplete || task.completed) ? "opacity-50" : ""}`}>
                      <div className="w-16 shrink-0 text-right">
                        <span className="text-xs font-black">{task.dueTime ? formatTime(task.dueTime) : "—"}</span>
                      </div>
                      <div className="w-px self-stretch bg-current opacity-20" />
                      <div className="flex-1 min-w-0">
                        <p className={`font-bold text-sm ${(task.isComplete || task.completed) ? "line-through" : ""}`}>{task.title}</p>
                        {(task.className || task.class) && (task.className || task.class) !== "General" && (
                          <p className="text-[10px] font-bold uppercase tracking-widest opacity-60 mt-0.5">{task.className || task.class}</p>
                        )}
                        {(task.notes || task.description) && (
                          <p className="text-xs opacity-70 mt-1">{task.notes || task.description}</p>
                        )}
                      </div>
                      <button onClick={() => handleDelete(task.id)} className="shrink-0 opacity-40 hover:opacity-100 transition-opacity">
                        <X size={14} />
                      </button>
                    </div>
                  ))
          ...
          <div className="bg-[var(--bg-header)] border border-[var(--border-color)] rounded-3xl shadow-2xl w-full max-w-md p-8 relative">
            ...
            <div className="space-y-6 pt-2">
              <div className="border-b border-slate-100 pb-1">
                <input type="text" value={newTaskText} onChange={e => setNewTaskText(e.target.value)} placeholder="Task Title..."
                  className="w-full text-3xl font-black bg-transparent text-[var(--text-strong)] placeholder-slate-300 outline-none" autoFocus />
              </div>
              <div className="border-b border-slate-100 pb-1">
                <input type="text" value={newTaskClass} onChange={e => setNewTaskClass(e.target.value)} placeholder="Class (e.g. CGT 390)"
                  className="w-full text-lg font-medium bg-transparent text-slate-500 placeholder-slate-300 outline-none" />
              </div>
              <textarea value={newTaskDetails} onChange={e => setNewTaskDetails(e.target.value)} placeholder="Add task details..."
              ...
              <div className="space-y-2">
                <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] ml-1">Due Time <span className="normal-case font-medium">(optional)</span></h4>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <select value={newTaskPriority} onChange={e => setNewTaskPriority(e.target.value as CalPriority)}
              ...
              <button onClick={handleAdd} disabled={!newTaskText.trim()}
              ...
              <div className="text-center">
                <span className="text-[10px] font-bold text-slate-300 uppercase tracking-widest">
                  Scheduled for: {formatDisplayDate(selectedDate)}{newTaskTime ? ` at ${formatTime(newTaskTime)}` : ""}
                </span>
              </div>
            </div>
          </div>
