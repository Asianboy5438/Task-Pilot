...
// Matches the interface in your main dashboard exactly
interface Task {
  id: number;
  title: string;
...
export default function CalendarPage() {
  ...
  // Load the shared tasks from the dashboard safely
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        console.error("Failed to sync calendar tasks", e);
      }
    }
  }, []);
    ...
    <div className="min-h-screen bg-[var(--bg-main)] p-10 transition-colors duration-300">
      <div className="max-w-6xl mx-auto">
        <header className="flex justify-between items-center mb-10 text-[var(--text-strong)]">
          <div>
            <h1 className="text-4xl font-black tracking-tighter">Calendar View</h1>
          ...
          <div className="flex items-center gap-6 bg-[var(--bg-header)] p-2 px-4 rounded-2xl border border-[var(--border-color)]">
             <button onClick={() => setCurrentMonth(m => m - 1)} className="hover:text-indigo-500"><ChevronLeft/></button>
             <span className="font-black text-sm uppercase tracking-widest w-40 text-center">
                {MONTHS[currentMonth]} {currentYear}
             </span>
             <button onClick={() => setCurrentMonth(m => m + 1)} className="hover:text-indigo-500"><ChevronRight/></button>
          </div>
        </header>
