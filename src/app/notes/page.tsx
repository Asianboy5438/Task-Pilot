...
import {
  ...
  Bold, Italic, Underline, Pen, Eraser, X, FolderPlus, Loader2
} from "lucide-react";
...
export default function NotesPage() {
  ...
  // ── Fetch folders and notes on mount ──
  useEffect(() => {
    async function load() {
      setLoading(true);
  ...
  const stopDraw = async () => {
    ...
    if (canvasRef.current && selectedNote) {
      ...
      setNotes(prev => prev.map(n =>
        n.id === selectedNote.id ? { ...n, drawing: dataUrl } : n
      ));
      setSelectedNote(prev => prev ? { ...prev, drawing: dataUrl } : prev);
    }
  };
  ...
  const clearCanvas = async () => {
    ...
    setNotes(prev => prev.map(n =>
      n.id === selectedNote.id ? { ...n, drawing: null } : n
    ));
  };
  ...
  const createNote = async () => {
    ...
    const res = await fetch("/api/notes", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ title: newNoteTitle.trim(), folderId: newNoteFolderId }),
    });
    if (res.ok) {
      ...
      <div className="w-64 flex flex-col border-r border-[var(--border-color)] bg-[var(--bg-header)] shrink-0">
        ...
        <div className="p-4 border-b border-[var(--border-color)]">
          <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={() => setShowNewFolderModal(true)} className="p-1 hover:bg-[var(--bg-avatar)] rounded-md text-slate-400 hover:text-indigo-500 transition-colors">
              <FolderPlus size={15} />
            </button>
          </div>
          ...
          <div className="space-y-0.5">
            <button
              ...
              className={`w-full flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-semibold transition-all ${
                selectedFolderId === "all" ? "bg-indigo-500/10 text-indigo-400" : "text-slate-400 hover:bg-[var(--bg-avatar)] hover:text-[var(--text-strong)]"
              }`}
            >
              ...
              <div key={folder.id} className="group flex items-center">
                <button
                  ...
                  className={`flex-1 flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-semibold transition-all ${
                    selectedFolderId === folder.id ? "bg-indigo-500/10 text-indigo-400" : "text-slate-400 hover:bg-[var(--bg-avatar)] hover:text-[var(--text-strong)]"
                  }`}
                >
                ...
                <button onClick={() => deleteFolder(folder.id)} className="opacity-0 group-hover:opacity-100 p-1 mr-1 text-slate-500 hover:text-red-400 transition-all">
                  <X size={12} />
                </button>
              </div>
            ))}
        ...
        <div className="flex-1 overflow-y-auto p-3">
          <div className="flex items-center justify-between mb-3 px-1">
            ...
            <button onClick={() => setShowNewNoteModal(true)} className="p-1 hover:bg-[var(--bg-avatar)] rounded-md text-slate-400 hover:text-indigo-500 transition-colors">
              <Plus size={15} />
            </button>
          </div>
              ...
              <div key={note.id} className="group relative">
                <button
                  ...
                  className={`w-full text-left px-3 py-3 rounded-xl transition-all ${
                    selectedNote?.id === note.id ? "bg-indigo-500/10 border border-indigo-500/20" : "hover:bg-[var(--bg-avatar)] border border-transparent"
                  }`}
                >
                  <p className={`text-xs font-bold truncate ${selectedNote?.id === note.id ? "text-indigo-400" : "text-[var(--text-strong)]"}`}>
                    {note.title}
                  </p>
                  <p className="text-[10px] text-slate-500 truncate mt-0.5">{note.content || "Empty note"}</p>
                  <p className="text-[10px] text-slate-600 mt-1">{note.createdAt.split("T")[0]}</p>
                </button>
                <button onClick={() => deleteNote(note.id)} className="absolute right-2 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 p-1.5 text-slate-500 hover:text-red-400 hover:bg-red-500/10 rounded-lg transition-all">
                  <Trash2 size={13} />
                </button>
              </div>
            ))}
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        {selectedNote ? (
          <>
            <div className="h-14 border-b border-[var(--border-color)] flex items-center px-6 gap-3 bg-[var(--bg-header)] shrink-0">
              ...
              <div className="flex items-center gap-1 bg-[var(--bg-main)] rounded-lg p-1 border border-[var(--border-color)]">
                <button onClick={() => setIsDrawing(false)} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-bold transition-all ${!isDrawing ? "bg-indigo-600 text-white shadow" : "text-slate-400 hover:text-[var(--text-strong)]"}`}>
                  <Bold size={12} /> Write
                </button>
                <button onClick={() => setIsDrawing(true)} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-bold transition-all ${isDrawing ? "bg-indigo-600 text-white shadow" : "text-slate-400 hover:text-[var(--text-strong)]"}`}>
                  <Pen size={12} /> Draw
                </button>
              </div>
              ...
              {isDrawing && (
                <div className="flex items-center gap-3">
                  <div className="flex items-center gap-1">
                    {COLORS.map(c => (
                      <button key={c} onClick={() => { setPenColor(c); setIsErasing(false); }}
                        className={`w-5 h-5 rounded-full border-2 transition-all ${penColor === c && !isErasing ? "border-white scale-125" : "border-transparent"}`}
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <input type="range" min={1} max={20} value={penSize} onChange={e => setPenSize(Number(e.target.value))} className="w-20 accent-indigo-500" />
                  </div>
                  <button onClick={() => setIsErasing(!isErasing)} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${isErasing ? "bg-amber-500/20 text-amber-400 border border-amber-500/30" : "text-slate-400 hover:bg-[var(--bg-avatar)]"}`}>
                    <Eraser size={13} /> Eraser
                  </button>
                  <button onClick={clearCanvas} className="text-xs text-slate-400 hover:text-red-400 transition-colors">Clear</button>
                </div>
              )}
            ...
            <div className="px-10 pt-8 pb-4 bg-[var(--bg-main)] shrink-0">
              ...
              <p className="text-xs text-slate-500 mt-1">
                {folders.find(f => f.id === selectedNote.folderId)?.name ?? "Unfiled"} · {selectedNote.createdAt.split("T")[0]}
              </p>
            </div>
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-center px-8">
            ...
            <button onClick={() => setShowNewNoteModal(true)} className="flex items-center gap-2 px-5 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-bold rounded-xl transition-colors">
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center px-4">
          <div className="bg-[var(--bg-header)] border border-[var(--border-color)] rounded-2xl shadow-2xl w-full max-w-sm p-6">
            <div className="flex items-center justify-between mb-5">
              ...
              <button onClick={() => setShowNewNoteModal(false)} className="text-slate-400 hover:text-red-400 transition-colors"><X size={18} /></button>
            </div>
            <div className="space-y-4">
              <div>
                <label className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1.5 block">Title</label>
                <input autoFocus value={newNoteTitle} onChange={e => setNewNoteTitle(e.target.value)} onKeyDown={e => e.key === "Enter" && createNote()} placeholder="Note title..."
              ...
              <div>
                ...
                <select value={newNoteFolderId ?? ""} onChange={e => setNewNoteFolderId(e.target.value || null)}
              ...
              <button onClick={createNote} disabled={!newNoteTitle.trim()} className="w-full py-2.5 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-40 text-white text-sm font-bold rounded-lg transition-colors">
                Create Note
              </button>
            </div>
          </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center px-4">
          <div className="bg-[var(--bg-header)] border border-[var(--border-color)] rounded-2xl shadow-2xl w-full max-w-sm p-6">
            <div className="flex items-center justify-between mb-5">
              ...
              <button onClick={() => setShowNewFolderModal(false)} className="text-slate-400 hover:text-red-400 transition-colors"><X size={18} /></button>
            </div>
            <div className="space-y-4">
              <div>
                <label className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1.5 block">Folder Name</label>
                <input autoFocus value={newFolderName} onChange={e => setNewFolderName(e.target.value)} onKeyDown={e => e.key === "Enter" && createFolder()} placeholder="e.g. CGT 390"
                  className="w-full px-3 py-2.5 rounded-lg bg-[var(--bg-main)] border border-[var(--border-color)] text-[var(--text-strong)] placeholder-slate-600 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 transition" />
              </div>
              <button onClick={createFolder} disabled={!newFolderName.trim()} className="w-full py-2.5 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-40 text-white text-sm font-bold rounded-lg transition-colors">
                Create Folder
              </button>
            </div>
          </div>
